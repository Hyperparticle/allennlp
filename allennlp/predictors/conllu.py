...
@Predictor.register("conllu")
class ConlluPredictor(Predictor):
    """
    Predictor that takes in a sentence and returns a single set of UD conllu annotations for it.
    """
    ...
    def _predict_unknown(self, instance: Instance):
        ...
        Handle cases where the labels are present in the prediction but not in the training set
        :param instance: the instance containing a list of labels for each namespace
        """
        def replace_tokens(instance: Instance, namespace: str, replacement_token: str):
        ...
        replace_tokens(instance, "lemmas_tags", "↓0;d¦")
        replace_tokens(instance, "feats_tags", "_")
        replace_tokens(instance, "xpos_tags", "_")
        replace_tokens(instance, "upos_tags", "NOUN")
    ...
    def dump_line(self, outputs: JsonDict) -> str:
        """Instead of outputting json as normal, output the conllu annotation"""
        word_count = len([word for word in outputs["words"]])
        lines = zip(*[outputs[k] if k in outputs else ["_"] * word_count
                      for k in ["ids", "words", "lemmas.tags", "upos.tags", "xpos.tags", "feats.tags",
                                "deps.predicted_heads", "deps.predicted_dependencies"]])
